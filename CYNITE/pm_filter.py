...
async def auto_filter(client, msg, spoll=False):
    ...
    if imdb and imdb.get('poster'):
        try:
            ...
            logger.exception(e)
            fek = await message.reply_photo(photo=NOR_IMG, caption=cap, protect_content=True, reply_markup=InlineKeyboardMarkup(btn))
            try:
                if settings['auto_delete']:
                    await asyncio.sleep(DELETE_TIME)
                    await fek.delete()
        ...
        fuk = await message.reply_photo(photo=NOR_IMG, caption=cap, protect_content=True, reply_markup=InlineKeyboardMarkup(btn))
        try:
            if settings['auto_delete']:
                await asyncio.sleep(DELETE_TIME)
                await fuk.delete()
